...
SYSCALL_DEFINE5(solve_puzzle, __u32, threshold, __u32, puzzle, __u32, dns_ip, __u32, client_ip, __u8, puzzle_type) {
...
int do_check_puzzle(u8 type, u32 puzzle, u32 dns_ip, u32 nonce, u32 threshold, u32 policy_ip) {
    struct puzzle_policy* policy;
    printk(KERN_INFO "type : %u, puzzle : %u, nonce : %u", type, puzzle, nonce);
    if (puzzle_type == PZLTYPE_NONE)
        return 1;
    if (do_set_puzzle(nonce, puzzle, dns_ip, policy_ip, type) >= threshold)
       return 1;
    return 0;
}
EXPORT_SYMBOL(do_check_puzzle);
SYSCALL_DEFINE6(check_puzzle, __u8, type, __u32, puzzle, __u32, dns_ip, __u32, nonce, __u32, threshold, __u32, policy_ip)
{
...
u32 do_set_threshold(u32 ip, u32 threshold) {
    ...
    int ret = add_policy(ip, threshold);
    while (ret == -1) {
        add_policy(ip, threshold);
    }
	return 0;
}
